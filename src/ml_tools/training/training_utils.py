...
class MetricHistory:
    ...
    def append_epoch(self, *, epoch: int, phase: str, metrics: dict[str, float]):
        self.append(epoch=epoch, **{f"{phase}_{k}": v for k, v in metrics.items()})

    def update_best(self, *, epoch: int, value: float):
        self.set(best_epoch=epoch, best_val=value)
