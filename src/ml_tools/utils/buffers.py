...
class EpochLogitBuffer:
    ...
    def add(self, *, preds: torch.Tensor, labels: torch.Tensor,
        ...
        self._preds.append(preds.detach())
    ...
    def clear(self):
        self._preds.clear(); self._labels.clear()
    ...
    def _stitch_local(self) -> Dict[str, torch.Tensor]:
        dev = self._preds[0].device if self._preds else torch.device("cpu")
        out = {
            "preds": torch.cat(self._preds, dim=0) if self._preds else torch.empty(0, device=dev),
    ...
    def gather_to_rank0(self, *, cast_fp16: bool = True, reorder_by_indices: bool = False) -> Optional[Dict[str, torch.Tensor]]:
        ...
        preds = local["preds"]; labels = local["labels"]
        n_local = int(labels.shape[0])

        # Choose gather path
        if self.assume_equal_lengths:
            preds_all = self._gather_equal(preds)
            ...
            preds_all = self._gather_varlen(preds, n_local)
        ...
        if cast_fp16:
            preds_all = preds_all.to(torch.float16)
        payload = {
            "preds": preds_all.cpu(),
        ...
        if indices_all is not None:
            ...
            if reorder_by_indices and idx_cpu.numel() > 0:
                ...
                payload["preds"] = payload["preds"][order]
